...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: enrichments } = await supabase
      ...
      .select(
        'destination_id, section_tags, confidence_scores, generated_description, family_suitability, luxury_suitability, adventure_suitability, beach_suitability, cultural_suitability'
      )
...
async function refreshSection(slug: string, destinations: any[]): Promise<SectionResult> {
  ...
  try {
    ...
    const { error: upsertErr } = await supabase.from('section_cache').upsert(
      {
        section_slug: slug,
        data: items,
...
function generatePopularDestinations(destinations: any[]): ContentItem[] {
  return destinations
    .filter(
      (d) =>
        d._aiTags.includes('popular-destinations') ||
        d.primary_category === 'popular' ||
        d.popularity_score > 700
    )
    .sort((a, b) => {
...
function generateEditorsChoice(destinations: any[]): ContentItem[] {
  return destinations
    .filter(
      (d) =>
        d._aiTags.includes('editors-choice') || (d.is_featured && (d.editor_rating || 0) >= 4.5)
    )
    .sort((a, b) => {
...
function generateMustSee(destinations: any[]): ContentItem[] {
  return destinations
    .filter(
      (d) => d._aiTags.includes('must-see') || (d.is_featured && (d.editor_rating || 0) >= 4.5)
    )
    .sort((a, b) => {
...
function generateLuxuryEscapes(destinations: any[]): ContentItem[] {
  return destinations
    .filter(
      (d) =>
        d._aiTags.includes('luxury-escapes') ||
        d.budget_level >= 4 ||
        d.primary_category === 'luxury'
    )
    .sort((a, b) => {
...
function generateFamilyFriendly(destinations: any[]): ContentItem[] {
  return destinations
    .filter(
      (d) =>
        d._aiTags.includes('family-friendly') ||
        d.best_for?.includes('families') ||
        d.primary_category === 'family'
    )
    .sort((a, b) => {
...
function generateBeachIslands(destinations: any[]): ContentItem[] {
  return destinations
    .filter(
      (d) =>
        d._aiTags.includes('beach-islands') ||
        d.primary_category === 'beach' ||
        d.tags?.includes('beach')
    )
    .sort((a, b) => {
