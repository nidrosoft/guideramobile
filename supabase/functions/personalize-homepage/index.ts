...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { user_id, sections: rawSections, section_cache_version, lat, lng, timezone } = body;
    ...
    const authenticatedUserId = await getUserIdFromRequest(
      req,
    ...
    // the Clerk-synced profile UUID in the body. Supabase DB/Storage requests
    // receive Clerk tokens from the client bridge, but Edge Functions do not.
    // Keep accepting JWT/service-role callers, and for anon mobile calls only
    // proceed after confirming the requested profile exists.
    if (!isServiceRoleCall && authenticatedUserId && authenticatedUserId !== user_id) {
      return new Response(
        JSON.stringify({ success: false, error: 'Valid authentication required' }),
        {
          status: 401,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
    ...
    if (!isServiceRoleCall && !authenticatedUserId) {
      const { data: profileExists, error: profileLookupError } = await supabase
        .from('profiles')
        .select('id')
        .eq('id', user_id)
        .maybeSingle();

      if (profileLookupError || !profileExists?.id) {
        return new Response(
          JSON.stringify({ success: false, error: 'Valid authentication required' }),
          {
            status: 401,
    ...
    const cacheVersion = String(section_cache_version || 'default');

    // 1. Build user profile. This also falls back to the saved onboarding
    // profile location when the client cannot provide live GPS coordinates.
    const profile = await buildUserProfile(user_id, lat, lng, timezone);
    const locationSignature = getLocationSignature(profile.location);

    const cached = await getPersonalizationCache(user_id, cacheVersion, locationSignature);
    if (cached) {
      return new Response(
        JSON.stringify({
          success: true,
    ...
    // 2. Cold-start users still need location-aware ordering. Re-rank the
    // cached global sections by proximity before applying lightweight section
    // preferences, but avoid generating a "For You" section until there is
    // enough behavioral signal.
    if (profile.strategy === 'cold') {
      const allDestinations = profile.location ? await fetchDestinationsWithEnrichment() : [];
      const locationRankedSections =
        allDestinations.length > 0
          ? rerankSectionItems(rawSections || [], allDestinations, profile)
          : rawSections || [];
      const coldSections = applyColdStartPersonalization(locationRankedSections, profile);
      return respond(coldSections, profile, startTime, cacheVersion);
    }
    ...
    return respond(finalSections, profile, startTime, cacheVersion);
  } catch (error: any) {
...
async function getPersonalizationCache(
  ...
  const { data } = await supabase
    ...
    .select('sections, meta, expires_at')
    .eq('user_id', userId)
    .eq('section_cache_version', cacheVersion)
    .gt('expires_at', new Date().toISOString())
    .maybeSingle();

  if (!data) return null;
  const cachedLocationSignature = data.meta?.locationSignature ?? null;
  return cachedLocationSignature === locationSignature ? data : null;
}

function respond(sections: any[], profile: UserProfile, startTime: number, cacheVersion: string) {
  const meta = {
    ...
    responseTimeMs: Date.now() - startTime,
    locationSignature: getLocationSignature(profile.location),
  };

  const cacheWrite = supabase.from('homepage_personalization_cache').upsert(
    {
    ...
    { onConflict: 'user_id,section_cache_version' }
  );
  const edgeRuntime = (
  ...
  if (edgeRuntime?.waitUntil) {
    ...
    cacheWrite.catch((error: any) =>
      console.warn('[personalize-homepage] cache write failed:', error?.message || error)
    );
  }
...
async function buildUserProfile(
  ...
  const [profileResult, prefsResult, interactionsResult, savedResult] = await Promise.all([
    supabase
      .from('profiles')
    ...
    supabase
      ...
      .limit(500),
    supabase.from('user_saved_items').select('destination_id').eq('user_id', userId),
  ]);
  ...
  const effectiveLat = parseCoordinate(lat) ?? parseCoordinate(savedProfile?.latitude);
  const effectiveLng = parseCoordinate(lng) ?? parseCoordinate(savedProfile?.longitude);
  const effectiveTimezone = timezone || savedProfile?.timezone;
  ...
  for (const ix of interactions) {
    ...
    const weight =
      ix.interaction_type === 'detail_view'
        ? 5
        : ix.interaction_type === 'save'
          ? 8
          : ix.interaction_type === 'share'
            ? 6
            : 1; // view/impression
  ...
  const savedIds = new Set<string>(
    savedItems.map((s: any) => s.destination_id as string).filter(Boolean)
  );
    ...
    if (topDests && topDests.length > 0) {
      ...
      for (const d of topDests) {
        ...
        if (d.primary_category)
          categoryCounts[d.primary_category] = (categoryCounts[d.primary_category] || 0) + 1;
        if (d.tags) {
  ...
  const hour = getLocalHour(effectiveTimezone);
  const timeOfDay =
...
function getLocationSignature(location: UserProfile['location']): string | null {
...
async function fetchDestinationsWithEnrichment(): Promise<any[]> {
  ...
  const { data: enrichments } = await supabase
    ...
    .select(
      'destination_id, section_tags, confidence_scores, generated_description, family_suitability, luxury_suitability, adventure_suitability'
    );
...
function generateForYouSection(
  ...
  const items = topPicks.map(({ dest, score, reasons }) =>
    formatForYouItem(dest, score, reasons, profile)
  );
...
function scorePreferenceMatch(dest: any, profile: UserProfile): number {
  ...
  if (
    profile.companionType === 'couple' &&
    (dest.best_for?.includes('couples') || dest.primary_category === 'romantic')
  )
    pts += 5;
...
function buildMatchReasons(dest: any, profile: UserProfile): string[] {
  ...
  if (profile.budgetAffinity > 0 && dest.budget_level) {
    ...
    else if (diff === 1 && dest.budget_level < profile.budgetAffinity)
...
function rerankSectionItems(sections: any[], allDestinations: any[], profile: UserProfile): any[] {
