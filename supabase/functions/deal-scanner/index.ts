...
async function scanHotelDeals(batchSize: number): Promise<ScanResult> {
  ...
  try {
    ...
    const { data: dnaRecords } = await supabase.from('user_travel_dna').select('top_destinations');
...
async function scanExperienceDeals(batchSize: number): Promise<ScanResult> {
  ...
  if (!viatorApiKey) {
    return {
      scan_type: 'experiences',
      success: false,
      deals_found: 0,
      deals_cached: 0,
      users_matched: 0,
      errors: ['VIATOR_API_KEY not configured'],
      duration_ms: Date.now() - startTime,
    };
  }
        ...
        for (const product of products.slice(0, 3)) {
          ...
          const images = (product.images || [])
            .map((img: any) => {
              ...
              const best =
                variants.find((v: any) => v.width === 720) ||
          ...
          const bookingUrl =
            product.productUrl || `https://www.viator.com/tours/${product.productCode}`;
...
function scoreDeal(
  ...
  if (pctBelowMedian >= 0.4) priceDropScore = 100;
  ...
  else if (pctFromLow <= 0.1) historicalLowScore = 70;
  else if (pctFromLow <= 0.2) historicalLowScore = 40;

  // Freshness (always 100 for new scans, weight 20%)
  const freshnessScore = 100;

  const score = Math.round(
    priceDropScore * 0.45 + historicalLowScore * 0.35 + freshnessScore * 0.2
  );

  // Badges
  const badges: string[] = [];
  if (currentPrice <= lowest) badges.push('record_low');
  else if (pctFromLow <= 0.1) badges.push('near_record_low');
...
async function matchDealsToUsers(homeAirport: string, _scanType: string): Promise<number> {
  ...
  try {
    ...
    const { data: users } = await supabase
      .from('user_travel_dna')
      .select(
        'user_id, top_destinations, budget_tier, max_flight_price, interest_vector, heritage_airport_codes, preferred_currency'
      )
    ...
    for (const user of users) {
      for (const deal of deals) {
        ...
        const channel =
          relevance.score >= 0.8 ? 'push' : relevance.score >= 0.5 ? 'feed' : 'digest';
...
async function matchHeritageDealsToUsers(): Promise<number> {
  ...
  try {
    ...
    const { data: users } = await supabase
      .from('user_travel_dna')
      .select(
        'user_id, home_airport_code, heritage_airport_codes, max_flight_price, budget_tier, interest_vector, top_destinations, preferred_currency'
      )
      .not('heritage_airport_codes', 'eq', '{}');
...
async function matchPopularDealsToUsers(): Promise<number> {
  ...
  try {
    ...
    const { data: users } = await supabase
      .from('user_travel_dna')
      .select(
        'user_id, home_airport_code, max_flight_price, budget_tier, interest_vector, top_destinations, heritage_airport_codes, preferred_currency'
      );
...
async function matchHotelDealsToUsers(): Promise<number> {
  ...
  try {
    ...
    const { data: users } = await supabase
      .from('user_travel_dna')
      .select(
        'user_id, home_city, max_hotel_nightly, budget_tier, interest_vector, top_destinations, preferred_currency'
      );
    ...
    for (const user of users) {
      for (const deal of deals) {
        ...
        const isTopDest = (user.top_destinations || []).some(
          (d: any) => (d.city || '').toLowerCase() === city.toLowerCase()
        );

        if (!isHomeCity && !isTopDest) continue;

        const priceRatio = user.max_hotel_nightly
          ? deal.price_amount / user.max_hotel_nightly
          : 0.7;
        let priceScore =
          priceRatio <= 0.5
            ? 1.0
        ...
        const score =
          destScore * 0.35 + priceScore * 0.3 + qualityScore * 0.2 + (isHomeCity ? 0.15 : 0);
...
async function matchExperienceDealsToUsers(): Promise<number> {
  ...
  try {
    ...
    const { data: users } = await supabase
      .from('user_travel_dna')
      .select(
        'user_id, home_city, budget_tier, interest_vector, top_destinations, preferred_currency'
      );
    ...
    for (const user of users) {
      for (const deal of deals) {
        ...
        const userInterests = (user.interest_vector || []).map((i: any) =>
          (i.tag || '').toLowerCase()
        );
        const dealTags = (deal.deal_data?.tags || []).map((t: string) => t.toLowerCase());
        if (
          userInterests.some(
            (i: string) => dealTags.includes(i) || i.includes('experience') || i.includes('tour')
          )
        ) {
          interestScore = 0.8;
        }
        ...
        await upsertDealMatch(
          user.user_id,
          deal,
          { score: Math.round(score * 100) / 100, reasons: reasons.slice(0, 3) },
          channel
        );
        matched++;
      }
...
function calculateRelevance(
  ...
  // 4. Heritage bonus (10%)
  const heritageScore = isHeritage
    ? 1.0
    : (userDna.heritage_airport_codes || []).includes(dest)
      ? 0.9
      : 0.0;
...
async function queueDealNotification(
  userId: string,
  notification: {
  ...
  try {
    ...
    if (dna) {
      ...
      if (dna.last_notification_at) {
        const hoursSince =
          (Date.now() - new Date(dna.last_notification_at).getTime()) / (1000 * 60 * 60);
