...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // SEC-01: Validate user identity from JWT; never trust body-provided user ids.
    ...
    if (!message?.trim() || !userId)
      return new Response(JSON.stringify({ error: 'message and valid authentication required' }), {
        status: 400,
    ...
    if (!sid) {
      const { data, error } = await sb
        ...
        .insert({
          ...
          context_id: contextData?.id || null,
          context_name: contextData?.name
            ? `${contextData.name}${contextData.location ? ', ' + contextData.location : ''}`
            : null,
    ...
    await sb
      .from('ai_chat_messages')
      .insert({ session_id: sid, role: 'user', content: message.trim() });

    // Premium gate
    const lower = message.toLowerCase();
    if (PREMIUM_KW.some((k: string) => lower.includes(k))) {
      const feat = lower.includes('packing')
        ? 'packing list'
      ...
      await sb
        .from('ai_chat_messages')
        .insert({ session_id: sid, role: 'assistant', content: upsell });
      return new Response(
        JSON.stringify({ sessionId: sid, response: upsell, isPremiumUpsell: true }),
        { headers: { ...cors, 'Content-Type': 'application/json' } }
      );
    }
        ...
        try {
          while (rounds < 4) {
            ...
            if (tools.length > 0) {
              ...
              rounds++;
              if (txts.length > 0 && r.stop_reason === 'end_turn') {
                text = txts.map((b: any) => b.text).join('');
                break;
              }
              continue;
            } else {
          ...
          if (text) {
            console.log(`Used ${model} (${rounds} tool rounds)`);
            break;
          }
        } catch (e: any) {
    ...
    if (!text && xk) {
      try {
        const r = await fetch('https://api.x.ai/v1/chat/completions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${xk}` },
          body: JSON.stringify({
            model: 'grok-3-mini',
        ...
        if (r.ok) {
          ...
          if (text) console.log('Used grok-3-mini');
        }
      } catch (e: any) {
        console.warn('xAI error:', e.message);
      }
    }
    ...
    return new Response(
      JSON.stringify({ sessionId: sid, response: text, isPremiumUpsell: false }),
      { headers: { ...cors, 'Content-Type': 'application/json' } }
    );
  } catch (err) {
