...
const COUNTRY_TO_HERITAGE_AIRPORTS: Record<string, string[]> = {
  cameroon: ['DLA', 'NSI', 'YAO'],
  cameroonian: ['DLA', 'NSI', 'YAO'],
  nigeria: ['LOS', 'ABV', 'PHC'],
  nigerian: ['LOS', 'ABV', 'PHC'],
  ghana: ['ACC'],
  ghanaian: ['ACC'],
  kenya: ['NBO'],
  kenyan: ['NBO'],
  'south africa': ['JNB', 'CPT'],
  'south african': ['JNB', 'CPT'],
  ethiopia: ['ADD'],
  ethiopian: ['ADD'],
  tanzania: ['DAR'],
  tanzanian: ['DAR'],
  ...
  australian: ['SYD', 'MEL'],
  canada: ['YYZ', 'YUL', 'YVR'],
  canadian: ['YYZ', 'YUL', 'YVR'],
  'united states': ['JFK', 'LAX', 'ORD'],
  american: ['JFK', 'LAX', 'ORD'],
};
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const user_id = await getUserIdFromRequest(
      req,
      body,
      supabaseUrl,
      supabaseServiceKey,
...
async function computeForUser(userId: string) {
  ...
  const [
    ...
    supabase
      ...
      .eq('is_expired', false)
      .limit(50),
    supabase.from('price_alerts').select('*').eq('user_id', userId).eq('is_active', true).limit(50),
    supabase
  ...
  const destinations = computeDestinationPreferences(
    searches,
    clicks,
    saved,
    alerts,
    geo.heritage_countries
  );
  ...
  const { error } = await supabase.from('user_travel_dna').upsert(dna, { onConflict: 'user_id' });
...
function computeGeographicIdentity(profile: any, searches: any[]) {
  ...
  return {
    ...
    home_airport_name: homeAirportName
      ? `${homeAirportName.charAt(0).toUpperCase() + homeAirportName.slice(1)} Airport`
      : null,
...
function computeDestinationPreferences(
  ...
  for (const c of clicks) {
    ...
    if (!destScores[key]) {
      destScores[key] = {
        city: snapshot.destination_city,
        code: snapshot.destination_code,
        country: snapshot.destination_country,
        score: 0,
        searches: 0,
        clicks: 0,
        saved: false,
...
function computeTravelPatterns(searches: any[]) {
  ...
  const weekendTrips = searches.filter((s) => {
    ...
    const length =
      (new Date(s.end_date).getTime() - new Date(s.start_date).getTime()) / (1000 * 60 * 60 * 24);
...
function computeCompanionProfile(preferences: any, searches: any[]) {
  ...
  if (searches.length >= 5) {
    const groupSizes = searches.map((s) => (s.adults || 1) + (s.children || 0) + (s.infants || 0));
...
function computeFlightProfile(preferences: any) {
  return {
    ...
    max_stops:
      preferences?.flight_stops === 'direct'
        ? 0
        : preferences?.flight_stops === 'one_stop'
          ? 1
          : null,
...
async function upsertSearchPatterns(userId: string, searches: any[], clicks: any[]) {
  // Top flight routes
  const flightRoutes: Record<
    string,
    { origin: string; dest: string; count: number; last: string }
  > = {};
  const hotelCities: Record<string, { city: string; country: string; count: number }> = {};

  let flightSearches = 0,
    hotelSearches = 0,
    experienceSearches = 0,
    carSearches = 0;
  ...
  const advanceDays = searches
    ...
    .map(
      (s) =>
        (new Date(s.start_date).getTime() - new Date(s.created_at).getTime()) /
        (1000 * 60 * 60 * 24)
    )
    .filter((d) => d > 0 && d < 365);
  ...
  await supabase.from('user_search_patterns').upsert(
    {
      ...
      avg_flight_price_clicked:
        flightClickPrices.length > 0 ? Math.round(avg(flightClickPrices)) : null,
      avg_hotel_price_clicked:
        hotelClickPrices.length > 0 ? Math.round(avg(hotelClickPrices)) : null,
      min_flight_price_clicked:
        flightClickPrices.length > 0 ? Math.min(...flightClickPrices) : null,
      max_flight_price_clicked:
        flightClickPrices.length > 0 ? Math.max(...flightClickPrices) : null,
